...
In this tutorial, we will skip the `r BiocStyle::Biocpkg("flowAI")` step.
...
+ When extracting `intensity_matrix`, it is labeled with channel names as the marker names
...
+ When multiple samples from the same batch or experiment run (samples are processed together), we recommend to borrow information from other samples by pooling all CD3+ across the same batch before applying the density gating.
